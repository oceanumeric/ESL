...
class OverviewSL:
    ...
    def plot_cube_neighbors(self):
        ...
        __drawCube(ax, [1.5, 1.5, 0.5], color='#18A75A')
        ax.grid(False)
        ax.view_init(17)
        ax.set_title(f'The share of unit cube and its neighbors is{5/5**3*100}%')
        ax = fig.add_subplot(122)
        x = np.linspace(0, 1, 5)
        ax.set_xlim(0, 5)
        ax.set_ylim(0, 5)
        ...
        ax.set_ylim(0, 5)
        ax.set_aspect('equal', adjustable='box')
        ax.plot(x, [1.5]*x.shape[0], '#18A75A')
        ax.plot([1.5]*x.shape[0], x, '#18A75A')
        x = np.linspace(0, 5, 10)
        xx = np.array([*product(x, x)])
        ax.scatter(xx[:,0], xx[:, 1],
                        facecolor='none', edgecolor='b')
        ax.set_title('The share of unit cube is 9% (it covers 9 points)')
